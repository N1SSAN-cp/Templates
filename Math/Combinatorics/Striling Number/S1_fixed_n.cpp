// Stirling Number of the First Kind - Fixed N
// [n, k] (unsigned) for all k in [0, n], given fixed n
// O(n log n) | n <= 2e5, mod = 998244353
// Problem: Count permutations of {1, 2, ..., n} with exactly k cycles, for every k.
//          Equivalently: coefficient of x^k in x(x+1)(x+2)...(x+n-1).
// Input:  n
// Output: [n,0] [n,1] ... [n,n]
...
// polynomial shift: f(x) -> f(x + c), O(n log n)
...
// computes (x+1)(x+2)...(x+n), O(n log n) for NTT-friendly primes
...
// Stirling Number of the First Kind - Fixed N, O(n log n)
// returns [n, k] for k in [0, n]
