...
// Lucas: O(log_MOD(n)), for n > MOD only
// Requires prime MOD
...
// Lucas for n > MOD — C(n,r) % p = C(n%p, r%p) * C(n/p, r/p) % p
long long lucas(long long n, long long r) {
    if (r == 0) { return 1; }
    return nCr(n % MOD, r % MOD) * lucas(n / MOD, r / MOD) % MOD;
}

int main() {
    combo_build();
    cout << nCr(10, 3)                         << "\n";  // 120
    cout << lucas(1000000000000000000LL, 2)    << "\n";  // large n > MOD
}
