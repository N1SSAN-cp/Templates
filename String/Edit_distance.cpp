...
// Edit Distance (Levenshtein) - O(nm) time, O(nm) space
int edit_distance_quadratic_memory(const string &S, const string &T) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      dp[i + 1][j + 1] =
          min({dp[i + 1][j] + 1, dp[i][j + 1] + 1, dp[i][j] + (S[i] != T[j])});
    }
  }
  return dp[n][m];
}

// Edit Distance (Levenshtein) - O(nm) time, O(m) space
...
// Edit Distance path reconstruction - O(nm) time, O(nm) space
